...
function App() {
    ...
    const { websites, refreshWebsites } = useWebsites(); // hook imported to get all websites
    ...
    const processedWebsites = useMemo(() => {
        ...
        return websites.map(website => {
            // Use WebsiteTicks from the API response
            const rawTicks = website.WebsiteTicks || [];
            const ticks = Array.isArray(rawTicks) ? rawTicks : [];
            // Sort ticks by creation time => sort in descending order => latest is at 0th index
            ...
            // Filtering All the ticks which is reported last 30 minutes 
            ...
            // where window can be good/bad/unknown => UptimeStatus array
            const windows: UptimeStatus[] = [];

            for (let i = 0; i < 10; i++) {
                const windowStart = new Date(Date.now() - (i + 1) * 3 * 60 * 1000);
                const windowEnd = new Date(Date.now() - i * 3 * 60 * 1000);
                //WindowTicks to be Considerd should be in the range of Window Start and End Time 
                ...
                // Convert GOOD/BAD to good/bad for comparison
            ...
            // Get the most recent status => last element => last window tick  
                ...
                <div className="space-y-4">
                    {processedWebsites.length === 0 ? (
                        <div className="text-center py-8">
                            <p className="text-gray-500 dark:text-gray-400">No websites found. Add a website to get started!</p>
                        </div>
                    ) : (
